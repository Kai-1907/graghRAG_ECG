...
# 3. 執行生成階段
# ---------------------------------------------------------
print("🚀 開始執行 GraphRAG 生成測試樣本...")
evaluation_data = []

for sample in tqdm(test_samples):
    response = call_graphrag(sample["input"])
    if response:
        evaluation_data.append({
            "question": sample["input"], # Ragas 預設欄位名稱建議用 question
            "contexts": [response],     # 預設用 contexts
            "answer": response,         # 預設用 answer
            "ground_truth": sample["reference"] # 預設用 ground_truth
        })
